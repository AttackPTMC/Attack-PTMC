    ...
    def beam_attack(self, original_bleu, example, substitutes, statement_dict, beam_size):
        ...
        for key, identifiers in statement_dict.items():
            if iter == 0:
                ...
                # init_list = []
                # for replace_info, value in init_pop.items():
                #     init_list.append(value["prob"])
                # select_probs = softmax(np.array(init_list))
                # # print(np.sum(select_probs))
                # # print("select_probs: ", select_probs)
                # final_keys = np.random.choice(np.array(list(init_pop.keys())), size=beam_size, p=select_probs)
                # final_pop = {k: init_pop[k] for k in final_keys}
