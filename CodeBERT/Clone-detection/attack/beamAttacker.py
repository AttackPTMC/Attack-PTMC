...
def get_statement_identifier(first_idx, identifiers):
    file = "../../../Identifier-Study/clone/data/data.csv"
    ...
    def greedy_attack(self, orig_prob, example, substitutes, code_pair, statement_dict):
        ...
        for greedy_identifier in greedy_identifiers:
            ...
            is_success, final_code, candidate, current_prob = self.perturb(tmp_example, tmp_code, words_2,
                                                                           substitutes[greedy_identifier], greedy_identifier)
            if candidate is not None:
                ...
                if is_success == 1:
                    ...
                    return result
                tmp_code = final_code
                temp_replace = " ".join(final_code.split())
                temp_replace = self.tokenizer_tgt.tokenize(temp_replace)
                new_feature = convert_examples_to_features(temp_replace, words_2, label, None, None, self.tokenizer_tgt,
                                                           self.args, None)
                tmp_example = CodeDataset([new_feature])[0]
            else:
                temp_replace = " ".join(tmp_code.split())
                temp_replace = self.tokenizer_tgt.tokenize(temp_replace)
                new_feature = convert_examples_to_features(temp_replace, words_2, label, None, None, self.tokenizer_tgt,
                                                           self.args, None)
                tmp_example = CodeDataset([new_feature])[0]
        ...
        for i_iter in range(len(adv_identifier)):
            ...
            for replace_info, value in final_pop_copy.items():
                ...
                for seq in value["sequence"]:
                    ...
                    temp_replace = self.tokenizer_tgt.tokenize(temp_replace)
                    new_feature = convert_examples_to_features(temp_replace, words_2, label, None, None, self.tokenizer_tgt, self.args, None)
                    example = CodeDataset([new_feature])
                    gaps = self.perturb(example[0], value["adv_code"], words_2, replace_dict[seq], seq, iter, equal=True)
                    flag += len(gaps)
                    if len(gaps) > 0:
                        gap = gaps[0]
                        is_success, final_code, candidate, current_prob = gap[0], gap[1], gap[2], gap[3]
